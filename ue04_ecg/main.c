...
void printHeartRate(double rpeak[], int length) {
  ...
  for(int i = 0; i < (length ); i++) {
    ...
    ausgabe =  (ECGDATA_SAMPLE_RATE * 60.0) /anzahl;
    printf("%lf\n", ausgabe);
  }
}
...
void analyseECG()
{
    ...
    while(1) {
        ...
        for(int i = last_qrs_start; i < ecg_length; i++) {

            ecg_compute[zaehler] = ecg_compute[last_qrs_start + zaehler];
            zaehler++;
        }
        ...
        while(readECG(ECGDATA_FILENAME, ecg_compute + zaehler, (ECGDATA_BUFFERSIZE - zaehler), &ecg_length) != NO_ERROR)
            ; // Daten werden richtig eingelesen

        ecg_length = ecg_length + zaehler;
        //readECG(ECGDATA_FILENAME, ecg + zaehler, (ECGDATA_BUFFERSIZE - zaehler), &ecg_length); // Daten werden richtig eingelesen
        ...
        //3.) QRS-Komplexe detektieren und Herzfrequenzen berechen
        offset = removeOffset(ecg,ecg_length);
        calcMovingAveraging(ecg, ecg_length);
        //changeFeld(ecg, ecg_compute, ecg_length);
        // nicht notwendig: changeFeld(ecg, ecg_compute, ecg_length);
        diffSignal(ecg, ecg_length);
        normalizeSignal(ecg, ecg_length);
        diffSignal(ecg, ecg_length);
        rpeak_length = findRPeak(ecg, ecg_compute, ecg_length, rpeak, rpeak_length, &last_qrs_start);
        markRPeak(rpeak, rpeak_length, rpeaksignal, rpeak_length);

        //4.) Wenn keine Pulse gefünden wurden Schock abgeben (Ausganbe des Textes Schock in der Konsole) wenn zwischen last qrs start und dem letzten Fekdelement größer sind als die TimeOut Zeit
        if( (ecg_length - last_qrs_start) > ecg_timeout )
        {
                printf ("SCHOCK!\n");
        }
